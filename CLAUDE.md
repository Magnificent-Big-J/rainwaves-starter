...
`tests/Feature/PresetInstallMatrixTest.php` goes one step further: `ModuleDisableTest` only ever flips one module while the rest stay at their default, and never touches `config:cache`/`route:cache` — production's actual boot path, distinct from the always-uncached `route:list` every other subprocess test uses. This one is generic over the module env-var list — billing × mobile × teams = 8 combinations, doubling automatically per module added later, nothing in the file itself needs to change — and for every combination genuinely installs — `config:cache` + `route:cache` + `migrate:fresh --seed` against a throwaway sqlite db — then asserts table presence matches each module's toggle. `config:cache`/`route:cache` write real files into `bootstrap/cache/` (unlike `route:list`/`migrate:fresh`, which never touch the working tree), so every combination unconditionally clears them again in a `finally` block — verified empirically that a full run leaves `bootstrap/cache/` exactly as it started, so a failed assertion can't leave a stale cached config/route file behind for whatever runs `php artisan` next, in CI or on a shared dev machine.

**The `teams=true, billing=false` combination is a genuine, handled exception in that matrix, not a gap.** Since Teams declares a real `dependencies()` on Billing, `bootstrap/providers.php`'s explicit check (see above) throws before *any* artisan command can boot the framework under that combination — including `config:clear`, so there's no "clean" command left to run first. The test branches on this up front (`$expectBootFailure`) and asserts the opposite of every other combination: that `config:cache` fails, and that its output actually names the violation, rather than failing for some unrelated reason. `ModuleDisableTest`'s own billing-focused cases still pin `MODULE_TEAMS_ENABLED=false` deliberately — they're testing Billing's toggle in isolation, and leaving Teams at its enabled default would make every `billing=false` case there fail for a reason unrelated to what those specific tests are about.

**Governance module boundary**: `App\Providers\Modules\GovernanceServiceProvider` owns `/v1/legal/*`, `/v1/governance/*` (`routes/modules/governance-api.php`), the `legal_acceptances`/`role_change_requests` migrations (`database/migrations/modules/governance/`), and the `LegalServiceInterface`/`GovernanceServiceInterface` bindings. Fully namespaced under `App\Modules\Governance\...` from day one, same RS-302 depth as Teams. Three genuinely separate sub-features scoped together in one pass (multi-select, same process used for Teams): legal consent tracking, fully-automated self-service data export/deletion, and a maker-checker approval workflow scoped to role *elevation* only (granting `admin`/`super-admin`) — demotions and every other role change apply immediately, unchanged.

**Governance declares no `dependencies()`, unlike Teams→Billing — a deliberate, different kind of module coupling.** `GovernanceService::exportDataFor()` reads `App\Modules\Teams\Models\TeamMembership` and `App\Modules\Billing\Models\Payment` directly, but only behind a runtime `ModuleRegistry::isEnabled('teams')`/`isEnabled('billing')` check — it degrades gracefully (that section of the export is just omitted) rather than requiring either module, the way Team::maxMembers() unconditionally requires Billing's Subscription table to exist. The rule this follows: a module may reference another module's classes for *optional, guarded* enrichment without declaring a hard dependency, as long as it never touches the other module's tables unless confirmed enabled first; `dependencies()` is reserved for the Teams-style case where the code assumes the other module's tables unconditionally exist.

**Role elevation is intercepted via a service decorator, not by modifying `UserAdminController`/`UserAdminService`.** `GovernanceServiceProvider::register()` rebinds `UserAdminServiceInterface` to `ApprovalGatedUserAdminService`, which wraps the real `UserAdminService` (injected *concretely*, not through its own interface, to avoid a binding cycle) and only overrides `create()`/`update()`: if the requested roles would newly grant `admin`/`super-admin`, those specific roles are stripped before delegating (so name/email/every other role change still applies immediately, never blocked), and a pending `RoleChangeRequest` is filed capturing the full originally-requested roles array. Because `bootstrap/providers.php` appends module providers after `AppServiceProvider`, this rebind naturally overrides `AppServiceProvider`'s own `bind()` for the same interface when Governance is enabled — and reverts to the plain `UserAdminService` with zero approval gating the moment it's disabled, since this provider (and its `register()` call) then never runs at all. `UserAdminController` itself was never touched — it still just calls `$this->service->update(...)`, unaware anything is intercepting it.

**Self-approval is blocked the same way self-role-change already is on the Teams side** — `GovernanceService::assertApprovable()` rejects `$roleChangeRequest->requested_by === $approver->id` with a `RuntimeException` → 422, and `governance.manage` (the permission gating the approval queue) is seeded to `super-admin` only, not `admin` — approving a privilege escalation should sit at least as high as what's being granted, not merely "any other admin."

**Data export/deletion reuse existing mechanisms rather than duplicating them.** `DELETE /v1/governance/account` calls the *same* `UserAdminService::archive()` admin-side archiving already uses (via the possibly-decorated `UserAdminServiceInterface` — `archive()`/`restore()` pass straight through the decorator unchanged), so the "last super-admin cannot be archived" guard applies for free with no separate rule to keep in sync. `GET /v1/governance/export` streams a JSON file rather than a queued/emailed one — the per-user dataset here is small enough (profile + a handful of optional sections) that a synchronous `streamDownload()` is the right level of complexity for a starter, matching the "fully automated self-service" scoping decision (no admin review step for either action).

**Legal consent tracking has no content-management system** — `pages/legal/{terms,privacy}.vue` stay static Vue pages; `config('governance.legal_versions')` is the only versioning mechanism, and bumping a document's version there is how a real content change would trigger re-acceptance for every user, checked via `LegalService::statusFor()` comparing each configured version against the user's latest `LegalAcceptance` row per document. No user-facing blocking route guard exists yet at the point of this backend phase — that's Phase 2, mirroring the existing unconditional `pendingTwoFactorRequired` redirect gate already in `router/index.js` rather than inventing a new class of enforcement; there is deliberately no equivalent *backend* middleware blocking every request on outstanding acceptance, matching the same frontend-level gate strength.

Proven with `tests/Feature/Api/LegalTest.php` (outstanding-status computation, acceptance recording, a version bump making a previously-accepted document outstanding again), `tests/Feature/Api/GovernanceTest.php` (export payload shape, self-delete reusing the last-super-admin guard for free, a second super-admin *can* still self-delete), and `tests/Feature/Api/RoleChangeRequestTest.php` (elevation via both `update()`/`create()` files a pending request instead of applying immediately, non-elevation and demotion changes still apply immediately with **no** request filed, approve/reject by a different admin, self-approval blocked, an already-decided request can't be re-approved, and `governance.manage` gating on the queue itself) — live-verified via a real Sail session: `MODULE_GOVERNANCE_ENABLED=false` confirmed to remove both `/v1/legal/*` and `/v1/governance/*` from `route:list` and `legal_acceptances`/`role_change_requests` from a fresh `migrate:fresh --seed`, not just trusted from the automated suite.

**Governance frontend (Phase 2)**: `stores/legal.js` (cached *per authenticated user*, not forever like `session`/`app-config` — legal status is per-user, so a logout+different-login in the same tab must refetch rather than trust the previous user's cached outstanding-documents state). `router/index.js`'s `beforeEach` gained an unconditional entry — same shape as the existing `pendingTwoFactorRequired` redirect — that awaits `legal.ensureLoaded()` for any authenticated session and redirects to the one new blocking page `pages/legal/accept.vue` when documents are outstanding, skipped when Governance is disabled. `profile.vue` gained a "Your data" section (it had no danger-zone card before this) — `GET /v1/governance/export` as a plain `v-btn :href=...` download link (Sanctum's session cookie flows with a normal anchor navigation, no JS fetch/blob handling needed, same as the existing `admin/users.vue` export link) and a delete-account `ConfirmDialog` that calls the endpoint then reuses `session.logout()` to clear local state (safe to call even though the backend already ended the session — its `finally` block clears `session.user` regardless of whether the API call itself 401s). `admin/governance.vue` — one `AppSectionCard` inline list (mirrors `account/team-members.vue`'s pending-invites list, not a full `AppDataTable`) with Approve/Reject buttons.

**A "Legal" column on `admin/users.vue` needed a second Governance endpoint, not a change to `UserAdminController`.** Giving admins per-user legal-acceptance visibility without making core aware of Governance meant a separate `GET /v1/governance/legal-acceptance-summary?ids[]=...` (gated on `users.view`, not `governance.manage` — this is read-only enrichment for the same audience that can already see the users list, not the elevation-approval surface), which `admin/users.vue` calls itself as a second, independent fetch after loading its own page — the same "core page conditionally calls an optional module's own endpoint" pattern `customer.vue`'s team switcher already established, just applied to `admin/users.vue` instead.

**Two real bugs found via live testing, not assumed:**

1. **`GovernanceService::exportDataFor()` checked `config('modules.teams')`/`config('modules.billing')` directly instead of `ModuleRegistry::isEnabled('teams'|'billing')`** — module state actually lives at `config('modules.enabled.*')` (see `bootstrap/providers.php`'s own comment, which documents this exact path), so both checks always silently resolved `null`/falsy and the `team_memberships`/`payments` export sections never appeared even with both modules enabled. `GovernanceService` now takes `ModuleRegistry` as a constructor dependency and calls `isEnabled()` like every other part of the app already does — caught by actually seeding a team/payment and checking the real export output, not by the original (too-loose) export test, which only asserted `profile`/`exported_at` were present. A regression test now asserts `team_memberships` actually appears.

2. **`WebConfigController` never reported Teams' enabled status** — `'modules' => ['billing' => ..., 'mobile' => ...]` was missing `teams` entirely (a gap that predates Governance, from Teams Phase 2), and since layouts' `hasModule()` treats an absent key as enabled (`!== false`, the deliberate fail-open default for a transient fetch failure), that silently meant Teams' nav items never actually hid when `MODULE_TEAMS_ENABLED=false`. Fixed alongside adding `governance`, so the same gap wasn't repeated for it.

**Seeded starter accounts are pre-accepted on the current legal documents — not via `StarterUsersSeeder` importing Governance.** "Ready to use starter accounts" (`StarterPlatformTest`) means not stuck behind the legal gate on first login either, but `StarterUsersSeeder` fires a new, generic `App\Events\StarterAccountsSeeded` event with zero awareness Governance exists, rather than resolving `LegalServiceInterface` itself — `GovernanceServiceProvider::boot()` listens for it, the same decoupled shape `LogSecurityActivity` already uses for lara-auth-suite's `UserRegistered` event. `LegalService::accept()` gained a `bool $log = true` parameter so this seed-time default doesn't also write a spurious `governance` activity-log entry — a real regression caught by the existing `ActivityLogTest` suite, which asserts on "most recent entry" ordering and broke the first time this ran, since a seed-time entry and a test's own entry can land in the same wall-clock second with no reliable tiebreaker.

**The unconditional legal-status check in the router guard measurably slows every authenticated navigation across the whole app by one (cached-after-first) request, and it surfaced real pre-existing timing fragility in the E2E suite** — not a Governance bug, but worth recording since it wasn't visible before. `tests/e2e/accessibility.spec.js`'s `assertNoViolations()` had no wait for network-idle before scanning; the added latency made it newly plausible for axe to catch a page mid-load with a transient, purely-cosmetic Vuetify `v-progress-linear` (an internal loading indicator) flagged for having no accessible name — fixed by waiting for `networkidle` before every scan, a general robustness fix, not a per-page patch. `tests/e2e/teams.spec.js`'s admin-overview assertion (`getByText('customer@rainwaves.test')`) was always ambiguous once the detail drawer fully renders (the owner's email legitimately appears in the table row, the owner stat card, *and* the members list) — it only ever "worked" because the drawer hadn't finished rendering yet at assertion time; scoped to `.detail-members__email` specifically. Confirmed stable across 4 consecutive full-suite runs after both fixes.

**`tests/e2e/governance.spec.js` (Phase 3)**: legal-gate redirect on a brand-new user + accepting clears it and stays cleared across a later navigation; self-service export (real file download) + self-delete; role elevation via `admin/users.vue`'s existing create-user form shows the elevated role stripped immediately, the pending request appears in `admin/governance.vue`, and self-approval is blocked with a real toast message (`.app-toast`, rendered by `AppToastHost.vue` off `useAppErrorsStore`). Deliberately doesn't exercise a genuine *different-admin* approval at the browser level — `governance.manage` is seeded to `super-admin` only and this starter seeds exactly one, so a true cross-admin approval can't be assembled through the UI alone without first approving a second super-admin into existence (a real, if narrow, bootstrapping property of the design, not a test gap) — that exact path is already thoroughly proven at the PHPUnit level (`RoleChangeRequestTest::test_a_different_admin_can_approve_a_pending_elevation`), consistent with this suite's own stated "smoke test, not exhaustive" scope.

**A real bug found writing this phase's E2E spec, not assumed**: `profile.vue`'s `confirmDeleteAccount()` called `session.logout()` (a full `/auth/session/logout` network round-trip) *after* `GovernanceController::deleteAccount()` had already ended the session server-side (`Auth::guard('web')->logout()` + session invalidation) — redundant, and live-testing showed it could actually hang the delete flow past the test's timeout, a real bug a live browser user would hit too, not just test friction. Fixed by calling `session.setUser(null)` directly instead — the backend already did the real work; the frontend only needs to stop believing it's authenticated.

`PresetInstallMatrixTest` extended to a fourth `MODULE_GOVERNANCE_ENABLED` axis — billing × mobile × teams × governance, 16 combinations, no new expected-failure branch since `GovernanceModule::dependencies()` is empty (every `governance=true/false` value installs cleanly regardless of the other three toggles), asserting `legal_acceptances` table presence matches the flag.
...
`npm run test:e2e` (Playwright, `playwright.config.js`, specs in `tests/e2e/`) — critical-path browser tests: guest homepage + nav, login (valid/invalid credentials, sign-out), admin user CRUD (create/archive/restore, non-admin denied), the users export button producing a real file download, the unsaved-changes guard blocking/allowing navigation correctly, and Teams (`tests/e2e/teams.spec.js` — create a team, invite and revoke a member, a second real admin-surface browser context sees it in the read-only overview, then the team is deleted; plus a non-admin-denied check). These are smoke tests, not exhaustive coverage — one or two representative flows per area, enough to catch "the app doesn't actually work" regressions that unit tests can't see (routing, real HTTP round-trips, Vuetify component interaction). `fixtures.js` gained a `customerPage` fixture (mirrors `ownerPage`) once Teams needed a logged-in customer session as its own first-class actor, not just an inline login block. The Teams spec starts by tidying up any team left over from an interrupted previous run before asserting the empty state — makes it deterministically re-runnable against a real (not-thrown-away) dev database, not just CI's fresh one.
