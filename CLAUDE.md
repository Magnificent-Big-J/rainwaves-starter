...
Advanced capabilities are optional modules rather than baseline coupling — Billing (PayFast) was the first; Mobile (the companion app's bootstrap/auth/device/sync API surface) is the second, built off the now-proven pattern. Disabling a module removes its routes, migrations, and frontend nav/UI; nothing needs to be deleted from the codebase to ship without it.
